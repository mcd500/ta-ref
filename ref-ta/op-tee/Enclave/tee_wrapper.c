...
TEE_Result _TEE_AsymmetricEncrypt(TEE_OperationHandle operation,
                 ...
                 uint32_t *destLen) {
    return TEE_AsymmetricEncrypt(operation, params, paramCount, srcData, srcLen, destData, destLen);
}
...
TEE_Result _TEE_AsymmetricDecrypt(TEE_OperationHandle operation,
				 ...
				 uint32_t srcLen, void *destData,
                 uint32_t *destLen) {
    return TEE_AsymmetricDecrypt(operation, params, paramCount, srcData, srcLen, destData, destLen);
}
